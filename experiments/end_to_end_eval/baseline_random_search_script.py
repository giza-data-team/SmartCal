...
def process_random_baseline(db: Session, table, output_path, checkpoint_steps=config_manager.num_itr_eval):
    ...
    for dataset_name in completed_datasets:
        ...
        for row in dataset_rows:
            ...
            best_config = {n: None for n in checkpoint_steps}
            best_ece = {n: float("inf") for n in checkpoint_steps}
            best_classification_metrics_calibrated = {n: None for n in checkpoint_steps}
            best_cal_metrics_calibrated = {n: None for n in checkpoint_steps}
            best_classification_metrics_test = {n: None for n in checkpoint_steps}
            best_cal_metrics_calibrated_test = {n: None for n in checkpoint_steps}
            random_combinations = get_n_random_calibration_combinations(all_combinations, max(checkpoint_steps), seed)
            ...
            for i, calibrator_dict in enumerate(random_combinations):
                try:
                    ...
                    print(f"Trial {i+1}/{max(checkpoint_steps)} for dataset {dataset_name} - {classification_model} - {calibrator_dict} - seed={seed}: n={i+1}, ECE={cal_metrics_calibrated['ece']}")
                    ...
                    # Track the best configuration after each checkpoint
