...
def generate_feature_importance_insights(feature_importance, feature_names, model_name, metric_name, output_dir):
    """Generate feature importance visualizations and save data"""
    safe_metric = re.sub(r'[^\w\-_.]', '_', str(metric_name))
    safe_model = re.sub(r'[^\w\-_.]', '_', str(model_name))
    ...
    # Dynamic figure height based on number of features for better readability
    fig_height = max(8, num_features * 0.4)  # At least 8 inches, then 0.4 inches per feature
    plt.figure(figsize=(12, fig_height))
    
    sns.barplot(data=importance_df, x='Importance', y='Feature', palette='viridis')
    plt.title(f'All Feature Importance ({num_features} features) - {model_name} on {metric_name}', fontsize=14)
    plt.xlabel('Importance Score', fontsize=12)
    plt.ylabel('Features', fontsize=12)
    plt.tight_layout()
    
    # Save the plot
    plot_path = os.path.join(importance_dir, f"{safe_metric}_{safe_model}_feature_importance.png")
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"💾 Saved feature importance visualization for {model_name} on {metric_name}")
...
def train_and_evaluate_models(df_metric, metric_name):
    ...
    for model_name, model_proto in MODEL_LIST.items():
        ...
        try:
            ...
            if hasattr(clf_multi, 'feature_importances_'):
                feature_importance = clf_multi.feature_importances_
                importance_df = generate_feature_importance_insights(
                    feature_importance, X.columns.tolist(), model_name, metric_name, CONFIG['insights_folder_path']
                )
                
                # Print top 10 most important features
                print(f"🔍 Top 10 Most Important Features for {model_name} on {metric_name}:")
...
def main():
    ...
    print(f"Feature importance visualizations saved to: {CONFIG['insights_folder_path']}/feature_importance/")
