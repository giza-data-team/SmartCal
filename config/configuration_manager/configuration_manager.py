...
from .configuration_manager_factory import ConfigManagerFactory
from ..enums.config_reader_enum import ConfigReaderEnum
...
class ConfigurationManager(ConfigurationManagerBase):
    ...
    @property
    def config_img(self):
        return str(self.config('CONFIG_IMG', default='Data_info.xlsx'))

    @property
    def config_tabular(self):
        return str(self.config('CONFIG_TABULAR', default='Data_info.xlsx'))

    @property
    def config_language(self):
        return str(self.config('CONFIG_LANGUAGE', default='Data_info.xlsx'))
    ...
    @property
    def imax_cal_mode(self):
        return str(self.config('IMAX_CAL_MODE', default='SCW'))

    @property
    def imax_num_bins(self):
        return int(self.config('IMAX_NUM_BINS', default=15))

    @property
    def imax_binning_stage(self):
        return str(self.config('IMAX_BINNING_STAGE', default='raw'))

    @property
    def imax_binning_repr_scheme(self):
        return str(self.config('IMAX_BINNING_REPR_SCHEME', default='pred_prob_based'))

    @property
    def imax_bin_init_mode(self):
        return str(self.config('IMAX_BIN_INIT_MODE', default='quantile'))
    ...
    @property    
    def imax_cal_mode_tune(self):
        cal_modes = self.config('IMAX_CAL_MODE_TUNE')
        cal_modes = cal_modes.strip("'[]'")
        return [mode.strip().strip("'") for mode in cal_modes.split(',')]
    
    @property    
    def imax_q_binning_stage_tune(self):
        binning_stages = self.config('IMAX_Q_BINNING_STAGE_TUNE')
        binning_stages = binning_stages.strip("'[]'")
        return [stage.strip().strip("'") for stage in binning_stages.split(',')]
    
    @property    
    def imax_bin_init_mode_tune(self):
        bin_init_modes = self.config('IMAX_BIN_INIT_MODE_TUNE')
        bin_init_modes = bin_init_modes.strip("'[]'")
        return [mode.strip().strip("'") for mode in bin_init_modes.split(',')]
    ...
    @property
    def n_calibrators(self):
        return int(self.config('N_CALIBRATORS', default=3))
