...
class TestEmpiricalBinningCalibrator(unittest.TestCase):
    ...
    def test_multiclass_monotonicity(self):
        """Test if calibration preserves ordering in multiclass settings."""

        # Create structured ordered probabilities (3-class case)
        probs = np.linspace(0.1, 0.9, 10)  # 10 ordered probability values
        ordered_pred = np.column_stack([1 - probs, probs * 0.5, probs * 0.5])

        # Adjust to ensure proper probability distribution
        scaling_factors = (1 - ordered_pred[:, 0])[:, np.newaxis] / ordered_pred[:, 1:].sum(axis=1, keepdims=True)
        ordered_pred[:, 1:] *= scaling_factors

        # Structured labels to avoid random noise
        structured_labels = np.tile(np.arange(3), 10 // 3 + 1)[:10]

        # Fit and predict
        self.calibrator.fit(ordered_pred, structured_labels)
        calibrated = self.calibrator.predict(ordered_pred)

        # Check if ordering is preserved for each class
        for class_idx in range(3):
            calibrated_probs = calibrated[:, class_idx]

            # Allow small floating-point errors
            self.assertTrue(
                np.allclose(np.diff(calibrated_probs), np.maximum(0, np.diff(calibrated_probs)), atol=1e-6),
                f"Calibration did not preserve monotonicity for class {class_idx}"
            )
