...
class TestSmartCal(unittest.TestCase):
    ...
    @patch("smartcal.meta_model.meta_model.MetaModel")
    @patch("smartcal.meta_features_extraction.meta_features_extraction.MetaFeaturesExtractor")
    def test_recommend_calibrators(self, MockExtractor, MockModel):
        ...
        recommendations = self.smartcal.recommend_calibrators(
            self.y_true, self.predictions_prob, n=2
        )

        self.assertEqual(len(recommendations), 2)
        self.assertAlmostEqual(sum(score for _, score in recommendations), 1.0)

    @patch("smartcal.bayesian_optimization.calibrators_bayesian_optimization.CalibrationOptimizer")
    @patch("smartcal.meta_model.meta_model.MetaModel")
    @patch("smartcal.meta_features_extraction.meta_features_extraction.MetaFeaturesExtractor")
    def test_best_fitted_calibrator(self, MockExtractor, MockBModel, MockOptimizer):
        ...
        with patch("smartcal.config.enums.calibration_algorithms_enum.CalibrationAlgorithmTypesEnum") as MockEnum:
            MockEnum.__getitem__.return_value = DummyCalibrator
            best_cal = self.smartcal.best_fitted_calibrator(self.y_true, self.predictions_prob, n_iter=10)
